"""Concatenate the page parts into build/manual.html and print it to PDF with headless Chrome."""
import glob
import json
import os
import re
...
HTML = os.path.join(BUILD, "manual.html")
PDF = os.path.join(ROOT, "Husqvarna_Automatic_21A_Operating_Manual_EN.pdf")
CHROME = r"C:\Program Files\Google\Chrome\Application\chrome.exe"


def mark_line_art(doc):
    """Every figure normalised to a white ground must be multiplied into the page."""
    ...
    def fix(m):
        ...
        if src not in white or 'class="mul"' in tag or "mul" in (re.search(r'class="([^"]*)"', tag) or [""])[0]:
            return tag
        if 'class="' in tag:
...
def assemble():
    ...
    with open(HTML, "w", encoding="utf-8") as fh:
        fh.write(doc)
    pages = sum(c.count('class="page') for c in chunks)
...
def to_pdf():
    ...
    print("wrote", os.path.relpath(PDF, ROOT), os.path.getsize(PDF) // 1024, "kB")
